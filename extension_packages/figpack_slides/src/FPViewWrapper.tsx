...
const FPViewWrapper: React.FC<{
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!zarrGroup) {
        setError("FPViewWrapper: No zarrGroup provided (2)");
        return;
      }
      ...
      setError(
        `Error rendering view s${zarrGroup.attrs.view_type} in FPViewWrapper: ${err}`,
      );
    }
  ...
  useEffect(() => {
    if (dataChangeCallbackRef.current) {
      if (!zarrGroup) {
        setError("FPViewWrapper: No zarrGroup provided (1)");
        return;
      }
      try {
