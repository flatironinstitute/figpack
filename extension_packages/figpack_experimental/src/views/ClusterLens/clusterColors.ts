...
let offset = 0; // so that we change which is the 0th cluster
let stride = 1;
let num: number | undefined = undefined; // so that we change the periodicity

export const getClusterColor = (clusterId: any) => {
  if (num === undefined) num = colors.length;
  const i = (offset + clusterId * stride) % num;
  return colors[i];
};

export const redistributeClusterColors = () => {
  if (num === undefined) num = colors.length;
  offset += 1; // so that we change which is the 0th cluster
