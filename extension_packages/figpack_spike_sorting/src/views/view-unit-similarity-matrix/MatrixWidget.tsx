import { BaseCanvas } from "../core-views/figurl-canvas";
...
const MatrixWidget: FunctionComponent<Props> = ({
  unitIds1,
  unitIds2,
  selectedUnitIds,
  onSetSelectedUnitIds,
  matrix,
  range,
  setHoveredInfo,
  width,
  ...
  const indToPixel = useMemo(
    () => (o: { i1: number; i2: number }) =>
  ...
  const pixelToInd = useMemo(
    () => (p: { x: number; y: number }) => {
      ...
      return 0 <= i1 && i1 < unitIds1.length && 0 <= i2 && i2 < unitIds2.length
        ? { i1, i2 }
        : undefined;
    },
    [unitIds1.length, unitIds2.length, size, offsetX, offsetY, affineTransform],
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (ind) {
        setHoveredInfo({
          unitId1: unitIds1[ind.i1],
          unitId2: unitIds2[ind.i2],
          value: matrix[ind.i1][ind.i2],
        });
      } else {
  ...
  return (
    ...
    >
      <BaseCanvas width={width} height={height} draw={paint} drawData={null} />
    </div>
  );
};
...
const colorForValue = (
  ...
  const a = Math.min(
    255,
    Math.max(0, Math.floor(((v - range[0]) / (range[1] - range[0])) * 255)),
  );
