...
const UnitSimilarityMatrixView: FunctionComponent<Props> = ({
  data,
  width,
  height,
}) => {
  const { selectedUnitIds, visibleUnitIds, unitIdSelectionDispatch } =
    useSelectedUnitIds();
  const [hoveredInfo, setHoveredInfo] = useState<HoveredInfo | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    unitIdSelectionDispatch({
      ...
      newUnitOrder: sortIds(data.unitIds),
    });
  }, [data.unitIds, unitIdSelectionDispatch]);

  const unitIdsFilt = useMemo(
    () =>
      data.unitIds.filter((u) => !visibleUnitIds || visibleUnitIds.includes(u)),
    [data.unitIds, visibleUnitIds],
  );
  ...
  const matrix = useMemo(() => {
    ...
    for (const x of data.similarityScores) {
  ...
  const handleSetSelectedUnitIds = useCallback(
    (x: (number | string)[]) => {
      unitIdSelectionDispatch({
        type: "SET_SELECTION",
      ...
      >
        <BottomToolbar
          ...
          onSelectSimilarUnits={
            selectedUnitIds.size === 1 ? handleSelectSimilarUnits : undefined
          }
        />
      </div>
    </div>
