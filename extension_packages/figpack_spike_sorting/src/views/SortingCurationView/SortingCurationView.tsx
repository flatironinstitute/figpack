...
const SortingCurationView: FunctionComponent<Props> = ({
  ...
  const { sortingCuration, sortingCurationDispatch, curating } =
    useSortingCuration();
  const {
    labelChoices = [],
    labelsByUnit = {},
  ...
  useEffect(() => {}, [effectiveLabelChoices]);

  // Calculate label selection state for each label
  const getLabelSelectionState = useCallback(
    (label: string): LabelSelectionState => {
  ...
  const isLabelInUse = useCallback(
    (label: string): boolean => {
      return Object.values(labelsByUnit).some((labels) =>
  ...
  const handleRemoveLabelChoice = useCallback(
    (labelToRemove: string) => {
      ...
      sortingCurationDispatch({
        type: "SET_LABEL_CHOICES",
        labelChoices: effectiveLabelChoices.filter(
  ...
  const findMergeGroupForUnit = useCallback(
    (unitId: number | string): (number | string)[] | undefined => {
  ...
  const checkboxContainerStyle: React.CSSProperties = {
    ...
    cursor:
      selectedUnitIdsArray.length > 0 && !isClosed ? "pointer" : "default",
        ...
        {selectedUnitIdsArray.length > 0 &&
          getSelectedUnitsMergeStatus.groupsInvolved.length > 0 && (
            <div style={{ marginBottom: "6px" }}>
              ...
              {getSelectedUnitsMergeStatus.groupsInvolved.map(
                (group, index) => {
        ...
        >
          <button
            ...
            style={{
              ...buttonStyle,
              backgroundColor:
                getSelectedUnitsMergeStatus.canMerge && !isClosed
                  ? "#d4edda"
                  : "#f8f9fa",
              borderColor:
                getSelectedUnitsMergeStatus.canMerge && !isClosed
            ...
            title={
              isClosed
                ? "Cannot merge when curation is closed"
                : selectedUnitIdsArray.length < 2
                  ? "Select 2 or more units to merge"
          ...
          <button
            ...
            style={{
              ...buttonStyle,
              backgroundColor:
                getSelectedUnitsMergeStatus.canUnmerge && !isClosed
                  ? "#fff3cd"
                  : "#f8f9fa",
              borderColor:
                getSelectedUnitsMergeStatus.canUnmerge && !isClosed
            ...
            title={
              isClosed
                ? "Cannot unmerge when curation is closed"
                : !getSelectedUnitsMergeStatus.canUnmerge
                  ? "Select units that are part of merge groups to unmerge"
        ...
        {selectedUnitIdsArray.length > 0 && (
          <div style={{ fontSize: "10px", color: "#666", marginTop: "4px" }}>
            {getSelectedUnitsMergeStatus.groupsInvolved.length > 0 && (
              <div>
                Selected units in{" "}
                {getSelectedUnitsMergeStatus.groupsInvolved.length} merge
                group(s)
              </div>
            )}
            {getSelectedUnitsMergeStatus.unmergedUnits.length > 0 && (
              <div>
                {getSelectedUnitsMergeStatus.unmergedUnits.length} unmerged
                unit(s) selected
              </div>
            )}
          </div>
            ...
            {!isClosed && (
              <div style={{ display: "flex", alignItems: "center" }}>
                <input
                  ...
                  onKeyPress={(e) =>
                    e.key === "Enter" && curating && handleAddLabelChoice()
                  }
