...
const paint = (ctxt: CanvasRenderingContext2D, props: PaintProps) => {
  ...
  // std dev shading
  if (pixelSpacePathsLower && pixelSpacePathsUpper)
    pixelSpacePathsLower.forEach((_p, ii) => {
  ...
  // outer percentile
  if (pp1 && pp1.length > 0 && pp4 && pp4.length > 0) {
    pp1.forEach((_p, ii) => {
  ...
  // inner percentile
  if (pp2 && pp2.length > 0 && pp2.length > 0 && pp3 && pp3.length > 0) {
    pp2.forEach((_p, ii) => {
