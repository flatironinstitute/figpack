...
type Props = {
  ...
  onSelectRect?: (
    ...
    selectedMarkerKeys: (string | number)[],
    o: { ctrlKey: boolean; shiftKey: boolean },
  ) => void;
  onClickPoint?: (
    p: { x: number; y: number },
    markerKey: string | number | undefined,
    o: { ctrlKey: boolean; shiftKey: boolean },
  ) => void;
};
...
const ScatterPlot: FunctionComponent<Props> = ({
  ...
  const handleSelectRect = useCallback(
    (
      ...
      if (onSelectRect) {
        onSelectRect(r0, selectedMarkerKeys, { ctrlKey, shiftKey });
      }
    },
  ...
  const handleClickPoint = useCallback(
    (
      ...
      if (onClickPoint) {
        onClickPoint(pixel2Coord(p), selectedMarkerKey, { ctrlKey, shiftKey });
      }
    },
...
const transformRect = (
  t: (p: { x: number; y: number }) => { x: number; y: number },
  r: { x: number; y: number; width: number; height: number },
) => {
