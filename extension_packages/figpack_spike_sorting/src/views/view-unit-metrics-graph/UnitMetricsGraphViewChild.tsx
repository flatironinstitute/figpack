...
import { determinePlotSizeForSquareMatrixGrid } from "../view-cross-correlograms/CrossCorrelogramsView";
...
const UnitMetricsGraphViewChild: FunctionComponent<Props> = ({
  data,
  width,
  height,
}) => {
    ...
    () =>
      units.sort((u1, u2) => {
        if (selectedUnitIds.has(u1.unitId) && !selectedUnitIds.has(u2.unitId)) {
          ...
          !selectedUnitIds.has(u1.unitId) &&
          selectedUnitIds.has(u2.unitId)
        ) {
          return -1;
  ...
  useEffect(() => {
    unitIdSelectionDispatch({
      ...
      newUnitOrder: sortIds(unitsSorted.map((u) => u.unitId)),
    });
  ...
  const plots: PGPlot[] = useMemo(() => {
    ...
    if (selectedUnitMetrics.length === 0) {
      ...
      const { plotWidth, plotHeight } = determinePlotSizeForSquareMatrixGrid(
        width - TOOLBAR_WIDTH - leftLabelWidth - 10,
        height - bottomLabelHeight - 10,
        selectedUnitMetrics.length,
      );
      const ret: PGPlot[] = [];
      for (const m1 of selectedUnitMetrics) {
        const metric1: UMGMetric | undefined = metrics.filter(
          (x) => x.key === m1,
        )[0];
        {
          const props: UnitMetricPlotProps = {
        ...
        for (const m2 of selectedUnitMetrics) {
          const metric2: UMGMetric | undefined = metrics.filter(
            (x) => x.key === m2,
          )[0];
          if (metric1 && metric2) {
            const props: UnitMetricPlotProps = {
      ...
      for (const m of selectedUnitMetrics) {
        const metric: UMGMetric | undefined = metrics.filter(
          (x) => x.key === m,
        )[0];
        const props: UnitMetricPlotProps = {
          type: "bottom-label",
  ...
  return (
    ...
    >
      ...
      <VerticalScrollView
        width={0}
        height={0}
        disableScroll={selectedUnitMetrics.length > 0 ? true : false}
      >
        <PlotGrid
          plots={plots}
