...
const UnitMetricHistogram: FunctionComponent<UnitMetricHistogramProps> = ({
  metric,
  metricRange,
  units,
  selectedUnitIds,
  ...
  const { bars, ticks, verticalLines } = useMemo(() => {
    const values = units
      .map((unit) => unit.values[metric.key])
      .filter((a) => a !== undefined)
      .map((a) => a as number);
    const valuesSelected = units
      .filter((u) => selectedUnitIds.has(u.unitId))
      .map((unit) => unit.values[metric.key])
      .filter((a) => a !== undefined)
      .map((a) => a as number);
    const unitIdsSelected = units
      .filter((u) => selectedUnitIds.has(u.unitId))
      .map((unit) => ({ unitId: unit.unitId, value: unit.values[metric.key] }))
      .filter((a) => a.value !== undefined)
      .map((a) => a.unitId);
    const colorsSelected = unitIdsSelected.map((u) => getUnitColor(idToNum(u)));
    return createHistogramBars(
      values,
      valuesSelected,
      colorsSelected,
      numBins || 10,
  ...
  const handleSelectRect = useCallback(
    (
      ...
      selectedBarKeys: (string | number)[],
      {
        ...
        xMin,
        xMax,
      }: { ctrlKey: boolean; shiftKey: boolean; xMin: number; xMax: number },
    ) => {
      if (shiftKey) {
        if (onZoomToRect) {
          onZoomToRect({
      ...
      for (const i of selectedBarKeys) {
...
const createHistogramBars = (
  ...
  for (const value of values) {
    const i = Math.min(
      Math.floor(((value - min) / (max - min)) * numBins),
      numBins - 1,
    );
    counts[i]++;
  }
...
const determineTickLocations = (min: number, max: number) => {
  ...
  return determineTickLocationsMsec(min * scale, max * scale).map(
    (v) => v / scale,
  );
};
