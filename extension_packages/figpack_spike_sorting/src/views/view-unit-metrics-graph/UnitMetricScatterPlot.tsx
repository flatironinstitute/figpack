...
const UnitMetricScatterPlot: FunctionComponent<UnitMetricScatterPlotProps> = ({
  metric1,
  metric2,
  metric1Range,
  metric2Range,
    ...
    for (const unit of units) {
      ...
      if (xValue !== undefined && yValue !== undefined) {
        ret.push({
          ...
          x: xValue,
          y: yValue,
          color: selectedUnitIds.has(unit.unitId)
  ...
  const handleSelectRect = useCallback(
    (
      ...
      selectedMarkerIds: (string | number)[],
      { ctrlKey, shiftKey }: { ctrlKey: boolean; shiftKey: boolean },
    ) => {
      if (shiftKey) {
        if (onZoomToRect) onZoomToRect(r);
        return;
      }
  ...
  const handleClickPoint = useCallback(
    (
      ...
      selectedMarkerKey: string | number | undefined,
      { ctrlKey, shiftKey }: { ctrlKey: boolean; shiftKey: boolean },
    ) => {
