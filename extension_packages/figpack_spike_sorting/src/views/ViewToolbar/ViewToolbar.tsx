...
const ToolbarButton: FunctionComponent<ToolbarElement> = (
...
const ToolbarDivider: FunctionComponent<ToolbarElement> = (
  props: ToolbarElement,
) => {
  return props.useHorizontalLayout ? <></> : <hr key={props.elementIndex} />;
};

const ToolbarText: FunctionComponent<ToolbarElement> = (
  props: ToolbarElement,
) => {
  ...
  const roundsToInt =
    Math.abs(numericContent - Math.round(numericContent)) *
      10 ** (sigFigs + 1) <
    1;
...
const ToolbarToggle: FunctionComponent<ToolbarElement> = (
  props: ToolbarElement,
) => {
  switch (props.subtype) {
    ...
    default:
      return (
        <span key={props.elementIndex}>
          ERROR: Bad toggle subtype {props.subtype}
        </span>
      );
  }
};

const ToolbarCheckbox: FunctionComponent<ToolbarElement> = (
  props: ToolbarElement,
) => {
  return (
    <Checkbox
...
const ToolbarSlider: FunctionComponent<ToolbarElement> = (
  props: ToolbarElement,
) => {
...
const ToolbarItem: FunctionComponent<ToolbarElement> = (
  props: ToolbarElement,
) => {
  switch (props.type) {
    case "button":
...
const ViewToolbar: FunctionComponent<Props> = (props) => {
  const rectifiedControls = useMemo(
    () => rectifyElements(props.customActions),
    [props.customActions],
  );
  const renderedControls = useMemo(
    () =>
      rectifiedControls.map((e, ii) => (
        <ToolbarItem
          key={ii}
          {...e}
