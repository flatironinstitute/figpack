...
import {
  ...
  detAffineTransform,
  useWheelZoom,
} from "../view-unit-similarity-matrix";
import { getUnitColor } from "../view-units-table/unitColors";
import { useSelectedElectrodes } from "../view-average-waveforms/WaveformWidget/sharedDrawnComponents/ElectrodeGeometry";
...
const SpikeLocationsWidget = (props: WidgetProps) => {
  ...
  const paintElectrodes = useCallback(
    (ctxt: CanvasRenderingContext2D, _props: any) => {
      const pixelRadius2 = pixelRadius * radiusScale;
      // set up fills
      const electrodesWithColors = pixelElectrodes.map((e) => {
        const selected = (selectedElectrodeIds || []).includes(e.e.id as never);
        const hovered = false;
        const dragged = false;
  ...
  const handleClickPoint = useCallback(
    (_x: Vec2, { ctrlKey: _ctrlKey }: { ctrlKey: boolean }) => {
