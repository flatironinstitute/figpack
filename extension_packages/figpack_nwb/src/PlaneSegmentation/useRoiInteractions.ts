...
export const useRoiInteractions = (
  ...
  const findRoiAtPosition = useCallback((canvasX: number, canvasY: number): number | null => {
    ...
    for (const boundingRect of roiBoundingRects) {
      if (canvasX >= boundingRect.minX && canvasX <= boundingRect.maxX &&
          canvasY >= boundingRect.minY && canvasY <= boundingRect.maxY) {
