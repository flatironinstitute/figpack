...
class TimeseriesGraph:
    ...
    def add_line_series(
        ...
        *,
        ...
        """
        self._series.append(TGLineSeries(
            name=name, t=t, y=y, color=color, width=width, dash=dash
        ))
    ...
    def add_marker_series(
        ...
        *,
        ...
        """
        self._series.append(TGMarkerSeries(
            name=name, t=t, y=y, color=color, radius=radius, shape=shape
        ))
    ...
    def add_interval_series(
        ...
        *,
        ...
        """
        self._series.append(TGIntervalSeries(
...
class TGLineSeries:
    def __init__(self, *, name: str, t: np.ndarray, y: np.ndarray, color: str, width: float, dash: Optional[List[float]]):
...
class TGMarkerSeries:
    def __init__(self, *, name: str, t: np.ndarray, y: np.ndarray, color: str, radius: float, shape: str):
        assert t.ndim == 1, "Time array must be 1-dimensional"
        assert y.ndim == 1, "Y array must be 1-dimensional"
        assert len(t) == len(y), "Time and Y arrays must have the same length"
        self.name = name
...
class TGIntervalSeries:
    def __init__(self, *, name: str, t_start: np.ndarray, t_end: np.ndarray, color: str, alpha: float):
        assert t_start.ndim == 1, "Start time array must be 1-dimensional"
        assert t_end.ndim == 1, "End time array must be 1-dimensional"
        assert len(t_start) == len(t_end), "Start and end time arrays must have the same length"
        assert np.all(t_start <= t_end), "Start times must be less than or equal to end times"
        self.name = name
