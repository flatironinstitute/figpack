#!/usr/bin/env python3
"""
Spike Sorting Tutorial Example Generator

Parses docs/spike_sorting_tutorial.md, extracts Python code blocks that precede iframe elements,
modifies them to save HTML files instead of showing interactively, and executes
them to generate the spike sorting tutorial example HTML files in docs/_build/html/spike_sorting_tutorial_*/
"""
...
def extract_iframe_and_code_pairs(tutorial_content: str) -> List[Tuple[str, str]]:
    ...
    while i < len(lines):
        ...
        if iframe_match:
            ...
            # Extract folder name from src like "./spike_sorting_tutorial_units_table/index.html?embedded=1"
            folder_match = re.match(r"\./?(spike_sorting_tutorial_[^/]+)/", src)
            if folder_match:
...
def modify_code_for_generation(code: str, folder_name: str) -> str:
    ...
    Args:
        ...
        folder_name: Target folder name (e.g., "spike_sorting_tutorial_units_table")
    ...
    def replace_show_call(match):
        ...
        title = (
            folder_name.replace("_", " ").replace("spike sorting tutorial ", "").title()
        )
...
def execute_code(code: str, folder_name: str) -> Tuple[bool, str, str]:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=120,  # 120 second timeout (longer for spike sorting)
...
def main():
    """Main function to generate all spike sorting tutorial examples."""
    print("🧠 Spike Sorting Tutorial Example Generator")
    print("=" * 50)

    # Read spike sorting tutorial file
    tutorial_path = Path(__file__).parent.parent / "spike_sorting_tutorial.md"

    if not tutorial_path.exists():
        print(f"❌ Spike sorting tutorial file not found: {tutorial_path}")
        sys.exit(1)
    ...
    try:
        ...
        print(f"❌ Error reading spike sorting tutorial file: {e}")
        sys.exit(1)
    ...
    if not pairs:
        print("❌ No iframe/code pairs found in spike sorting tutorial")
        sys.exit(1)
    ...
    if total - passed > 0:
        ...
        print(
            f"\n🎉 All {total} spike sorting tutorial examples generated successfully!"
        )
