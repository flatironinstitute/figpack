...
function showHelp() {
    console.log(`
figpack-utils.ts Checker and Propagator

USAGE:
  node scripts/check-figpack-utils.js [OPTIONS]
...
DESCRIPTION:
  This script finds all figpack-utils.ts files in the project and checks if they
...
EXAMPLES:
  ...
  node scripts/check-figpack-utils.js

  # Propagate changes from most recent file to others
  node scripts/check-figpack-utils.js --propagate
`);
}
...
function main() {
    ...
    const filePaths = findFiles('.', 'figpack-utils.ts');

    if (filePaths.length === 0) {
        console.log('No figpack-utils.ts files found');
        process.exit(0);
    }

    if (filePaths.length === 1) {
        console.log('Only one figpack-utils.ts file found:');
        console.log(`- ${filePaths[0]}`);
        console.log('\nNothing to check or propagate');
        process.exit(0);
    }

    console.log('Found figpack-utils.ts files:');
    ...
    if (!allOthersMatch) {
        console.error('\nError: The other figpack-utils.ts files (excluding the most recent) are not identical!');
    ...
    if (!mostRecentDifferent) {
        console.log('\nAll figpack-utils.ts files are already identical');
        process.exit(0);
    }
    ...
    if (propagate) {
        ...
        console.log('node scripts/check-figpack-utils.js --propagate');
