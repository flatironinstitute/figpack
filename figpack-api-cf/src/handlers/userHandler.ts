...
export async function handleRegenerateApiKey(
  ...
  try {
    ...
    if (action !== "regenerate") {
      return json(
        { success: false, message: 'Invalid action. Use "regenerate" to regenerate API key.' },
        400
      );
    }
    ...
    return json(
      { success: true, message: "API key regenerated successfully", user },
      200,
      {
        "X-RateLimit-Limit": "30",
        "X-RateLimit-Remaining": rateLimitResult.remaining.toString(),
  ...
  try {
    ...
    await env.figpack_db
      ...
      .bind(
        ...
        userData.researchDescription !== undefined
          ? userData.researchDescription
          : existing.research_description,
        Date.now(),
        userEmail
      )
      .run();
