...
export async function handlePinFigure(
  request: Request,
  env: Env,
  ...
  try {
    ...
    const { figureUrl, apiKey, pinInfo } = body;
    ...
    await env.figpack_db
      ...
      .bind(
        pinInfo.name.trim(),
        pinInfo.figureDescription.trim(),
        now,
        now,
        figureUrl
      )
...
export async function handleUnpinFigure(
  request: Request,
  env: Env,
  rateLimitResult: RateLimitResult
): Promise<Response> {
...
export async function handleRenewFigure(
  request: Request,
  env: Env,
  rateLimitResult: RateLimitResult
): Promise<Response> {
...
export async function handleRenewBulkFigures(
  request: Request,
  env: Env,
  rateLimitResult: RateLimitResult
): Promise<Response> {
