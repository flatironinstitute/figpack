...
export async function handleCreateDocument(request: Request, env: Env, rateLimitResult: RateLimitResult): Promise<Response> {
	try {
      ...
      `,
		...
		const newDocument = await env.figpack_db.prepare('SELECT * FROM figpack_documents WHERE id = ?').bind(result.meta.last_row_id).first();
...
export async function handleGetDocument(request: Request, env: Env, rateLimitResult: RateLimitResult): Promise<Response> {
	try {
		...
		// Find the document
		const docRow = await env.figpack_db.prepare('SELECT * FROM figpack_documents WHERE document_id = ?').bind(documentId).first();
...
export async function handleListDocuments(request: Request, env: Env, rateLimitResult: RateLimitResult): Promise<Response> {
	try {
		const apiKey = request.headers.get('x-api-key');
      ...
      `,
			...
			.bind(userEmail, `%"${userEmail}"%`, `%"${userEmail}",%`, `%,"${userEmail}"%`)
			.all();
...
export async function handleUpdateDocument(request: Request, env: Env, rateLimitResult: RateLimitResult): Promise<Response> {
	try {
		...
		// Check edit access
		const hasEditAccess = document.ownerEmail === userEmail || (document.editMode === 'users' && document.editorEmails.includes(userEmail));
		...
		const updatedRow = await env.figpack_db.prepare('SELECT * FROM figpack_documents WHERE document_id = ?').bind(documentId).first();
...
export async function handleDeleteDocument(request: Request, env: Env, rateLimitResult: RateLimitResult): Promise<Response> {
	try {
		...
		// Delete the document
		await env.figpack_db.prepare('DELETE FROM figpack_documents WHERE document_id = ?').bind(documentId).run();
...
export async function handleGetDocumentsReferencingFigure(request: Request, env: Env, rateLimitResult: RateLimitResult): Promise<Response> {
	try {
		const url = new URL(request.url);
      ...
      `,
		...
		const documents = result.results.map(parseDocument).filter((doc) => {
