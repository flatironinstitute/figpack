...
export async function getSignedUploadUrl(bucket: S3BucketConfig, key: string): Promise<string> {
...
export async function deleteObject(bucket: S3BucketConfig, key: string): Promise<void> {
	const client = createS3Client(bucket);
	const bucketName = bucketNameFromUri(bucket.uri);
...
export async function deleteObjects(bucket: S3BucketConfig, keys: string[]): Promise<void> {
