...
const data = fs
	.readFileSync('buckets.json', 'utf8')
	.split('\n')
	.filter(Boolean)
...
const sqlStatements = data.map((bucket) => {
	...
	const createdAt = bucket.createdAt
		...
		: Date.now();
	const updatedAt = bucket.updatedAt
...
try {
	...
	execSync(`npx wrangler d1 execute figpack-db --remote --file=import-buckets-batch.sql`, { stdio: 'inherit' });
