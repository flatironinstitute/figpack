...
const LockableSelectUnitsWidget: FunctionComponent<
  LockableSelectUnitsWidgetProps
> = (props: LockableSelectUnitsWidgetProps) => {
        ...
        <FormControlLabel
          control={
            <Switch
              checked={locked}
              size={"small"}
              onChange={() => toggleLockStateCallback()}
            />
          }
          label={<Typography variant="caption">Lock selection</Typography>}
        />
      </FormGroup>
      <SelectUnitsWidget {...props} selectionDisabled={locked} />
    </Fragment>
  );
};
