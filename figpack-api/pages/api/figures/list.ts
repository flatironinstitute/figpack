...
export default async function handler(
  ...
  try {
    ...
    if (status && ['uploading', 'completed', 'failed'].includes(status as string)) {
    ...
    const validSortFields = ['createdAt', 'updatedAt', 'expiration', 'figureUrl', 'status'];
    ...
    const figuresDocs: IFigureDocument[] = await Figure
      .find(filter)
      .sort(sortObj)
      .skip(skip)
      .limit(limitNum)
